...
def main():
    ...
    print_result(process_query(query), sys.argv[2])
    print "\n\nSearch completed"

def process(infile):
    
    global tokenized_file 
    tokenized_file = tokenize(infile)
    global inverted_file 
    inverted_file = inverted_index(tokenized_file)
...
def inverted_index(tokenized_file):
    all_tags = []
...
def print_result(query, outfile):
